...
export default function UserPortal() {
  ...
  return (
    <div style={{ maxWidth: 980, margin: '0 auto', padding: isMobile ? '0 2px' : 0 }}>
      {/* Header */}
      <div style={{ background: '#1c2333', border: '1px solid #2a3650', borderRadius: 10,
        ...
        <div style={{ width: 44, height: 44, borderRadius: '50%', background: '#3b82f6', flexShrink: 0,
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 700, color: '#fff' }}>
          {user?.fullName[0]}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: isMobile ? 14 : 15, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {user?.fullName}
          </div>
        ...
        <span style={{
          padding: '4px 10px', borderRadius: 20, fontSize: 11, fontWeight: 500, flexShrink: 0,
          background: isExempt ? 'rgba(34,197,94,.15)' : 'rgba(59,130,246,.15)',
          color: isExempt ? '#22c55e' : '#3b82f6',
        }}>{isExempt ? '✅ Miễn phí' : '💳 STANDARD'}</span>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(380px, 1fr))', gap: 14 }}>
        ...
        <div>
          <div style={{
            ...
            borderRadius: 12, padding: isMobile ? 16 : 22, color: '#fff', marginBottom: 14,
          }}>
            <div style={{ fontSize: 11, opacity: .8 }}>
              Dư nợ – {summary.currentPeriod}
            </div>
            <div style={{ fontSize: isMobile ? 28 : 36, fontWeight: 800, fontFamily: 'monospace', margin: '6px 0' }}>
              {isExempt ? 'Miễn phí' :
            ...
            <div style={{ background: '#1c2333', border: '1px solid #2a3650', borderRadius: 10, overflow: 'hidden', marginBottom: 14 }}>
              ...
              {pending.map((p: any) => (
                <div key={p.id} style={{ padding: '12px 14px', borderBottom: '1px solid rgba(42,54,80,.4)',
                  ...
                  justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: 10 }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600 }}>Kỳ {p.billingPeriod}</div>
                    <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>
                      {p.totalDuration} phút · <code style={{ color: STATUS_COLOR[p.status] }}>{p.status}</code>
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center', width: isMobile ? '100%' : 'auto', justifyContent: isMobile ? 'space-between' : 'flex-end' }}>
                    ...
                    <button onClick={() => handlePay(p.id, p.amount)} disabled={paying === p.id}
                      style={{
                        padding: isMobile ? '10px 18px' : '7px 14px', borderRadius: 8,
                        background: paying === p.id ? '#374151' : '#7c3aed',
                        color: '#fff', border: 'none', fontSize: isMobile ? 13 : 12, fontWeight: 600,
        ...
        <div>
          <div style={{ background: '#1c2333', border: '1px solid #2a3650', borderRadius: 10, overflow: 'hidden', marginBottom: 14 }}>
            ...
            <div style={{ padding: 12, minHeight: 80, maxHeight: isMobile ? 120 : 160, overflowY: 'auto', fontFamily: 'monospace', fontSize: 11 }}>
              {log.length === 0 ?
          ...
          <div style={{ background: '#1c2333', border: '1px solid #2a3650', borderRadius: 10, overflow: 'hidden' }}>
            ...
            <div style={{ maxHeight: isMobile ? 240 : 280, overflowY: 'auto' }}>
                  ...
                  <div key={p.id} style={{ padding: '11px 14px', borderBottom: '1px solid rgba(42,54,80,.4)',
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 10, color: '#64748b', fontFamily: 'monospace', marginTop: 1,
                        whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: isMobile ? 140 : 'none' }}>
                        {p.bkpayTxnId || '—'}
                      </div>
                    </div>
      ...
      <div style={{ background: '#1c2333', border: '1px solid #2a3650', borderRadius: 10, overflow: 'hidden', marginTop: 14 }}>
        ...
        <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' } as any}>
          ...
          {isMobile ? (
            history.length === 0 ?
              <div style={{ padding: 20, textAlign: 'center', color: '#64748b' }}>Không có dữ liệu</div> :
              history.slice(0, 20).map((s: any) => (
                <div key={s.id} style={{ padding: '12px 14px', borderBottom: '1px solid rgba(42,54,80,.4)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                    <span style={{ fontSize: 12, fontFamily: 'monospace', fontWeight: 600 }}>
                      {new Date(s.entryTime).toLocaleString('vi-VN', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' })}
                    </span>
                    <span style={{
                      padding: '2px 8px', borderRadius: 20, fontSize: 10,
                      background: s.status === 'CLOSED' ? 'rgba(34,197,94,.15)' : 'rgba(59,130,246,.15)',
                      color: s.status === 'CLOSED' ? '#22c55e' : '#3b82f6',
                    }}>{s.status === 'CLOSED' ? 'Đã ra' : 'Đang đỗ'}</span>
                  </div>
                  <div style={{ fontSize: 11, color: '#64748b', display: 'flex', gap: 12 }}>
                    <span>Slot: <b style={{ color: '#e2e8f0' }}>{s.slot?.slotCode || '—'}</b></span>
                    <span>{s.durationMinutes ? `${s.durationMinutes} phút` : '—'}</span>
                    {s.exitTime && <span>Ra: {new Date(s.exitTime).toLocaleTimeString('vi-VN', { hour:'2-digit', minute:'2-digit' })}</span>}
                  </div>
                </div>
              ))
          ) : (
