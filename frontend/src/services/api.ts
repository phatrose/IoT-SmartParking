  ...
  (err) => {
    if (err.response?.status === 401) {
      ...
      if (
        ...
        !window.location.pathname.includes("/cas")
      ) {
...
export const visitorApi = {
  ...
  checkout: (ticket_code: string) =>
    api.post("/api/visitor/checkout", { ticket_code }),
  list: (active = true) =>
    api.get("/api/visitor/tickets", { params: { active } }),
};

export const iotApi = {
  getAllSlots: (zone?: string) =>
    api.get("/api/iot/slots", { params: zone ? { zone } : {} }),
...
export const billingApi = {
  myBilling: (period?: string) =>
    api.get("/api/billing/me", { params: period ? { period } : {} }),
  payNow: (id: number) => api.post(`/api/billing/pay/${id}`),
  triggerCycle: (period: string) =>
    api.post("/api/billing/cycle", { billing_period: period }),
  getAllPayments: (filters?: any) =>
    api.get("/api/billing/payments", { params: filters }),
};
...
export const adminApi = {
  ...
  getReport: (period: string) =>
    api.get("/api/admin/report", { params: { period } }),
};
