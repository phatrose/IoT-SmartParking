...
export default function AdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    tasks.push(
      adminApi
        .getLogs({ limit: 50 })
        .then((l) => setLog(l.data))
        .catch(() => toast("Không tải được system log", "error")),
    );
    ...
    if (canPricing) {
      tasks.push(
        adminApi
          .getPricing()
          .then((p) => setPricing(p.data))
          .catch(() => toast("Không tải được bảng giá", "error")),
      );
    }
  ...
  useEffect(() => {
    if (!canPricing && tab === "pricing") setTab("users");
  }, [canPricing, tab]);
  ...
  const handleCreate = async () => {
    if (!form.hcmutId || !form.fullName || !form.password) {
      toast("Vui lòng điền đủ thông tin bắt buộc", "warning");
      return;
    }
    setSaving(true);
    try {
  ...
  const savePricing = async (p: any) => {
    try {
      await adminApi.upsertPricing({
        ...
        rate_per_hour: Number(p.ratePerHour),
        daily_cap: Number(p.dailyCap),
        is_exempt: p.isExempt,
      });
      toast("Đã lưu bảng giá", "success");
      load();
    } catch {
      toast("Lỗi lưu giá", "error");
    }
  };
  ...
  const exportCSV = () => {
    const rows = [
      "Time,Type,User,Description,Status",
      ...log.map(
        (l) =>
          `"${l.createdAt}","${l.eventType}","${l.userName || ""}","${l.description.replace(/"/g, '""')}","${l.status}"`,
      ),
    ];
    ...
    a.download = `SPMS_Log_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  };
  ...
  const filtered = users.filter((u) => {
    ...
    return (
      ...
      u.hcmutId?.toLowerCase().includes(q) ||
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 13, color: "#64748b", marginBottom: 20 }}>
        {tab === "users"
          ? "Thêm, sửa, xóa và quản lý tài khoản trong hệ thống"
          : tab === "pricing"
            ? "Thiết lập giá đỗ xe theo vai trò"
      ...
      >
        {(
          [
            ["users", "Người dùng"],
            ...(canPricing ? [["pricing", "Bảng giá"] as const] : []),
            ["log", "System Log"],
          ] as const
        ).map(([k, l]) => (
          <button
      ...
      {tab === "users" && (
        <div>
          ...
          <div
            style={{
              background:
                "linear-gradient(90deg,rgba(59,130,246,.2),rgba(59,130,246,.08))",
                ...
                {pageUsers.length === 0 ? (
                  ...
                  pageUsers.map((u) => {
                    ...
                    return (
                      <tr
                        key={u.id}
                        style={{ borderBottom: "1px solid rgba(42,54,80,.4)" }}
                      >
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <span
                            style={{
                              fontWeight: 700,
                              color: u.balance > 0 ? "#f59e0b" : "#22c55e",
                              fontSize: 13,
                            }}
                          >
                            {u.balance > 0
                              ? `-${Number(u.balance).toLocaleString("vi-VN")}đ`
                              : "0đ"}
                          </span>
                        </td>
                        <td style={{ padding: "12px 16px" }}>
      ...
      {tab === "pricing" && (
        ...
        >
          ...
          <div style={{ padding: 18 }}>
                ...
                .map((p) => (
                  ...
                  >
                    ...
                    <div style={{ marginBottom: 10 }}>
                      ...
                      <input
                        type="number"
                        defaultValue={Number(p.ratePerHour)}
                        onBlur={(e) =>
                          savePricing({ ...p, ratePerHour: e.target.value })
                        }
                        style={{
                          width: "100%",
                          background: "#1c2333",
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        defaultValue={Number(p.dailyCap)}
                        onBlur={(e) =>
                          savePricing({ ...p, dailyCap: e.target.value })
                        }
                        style={{
                          width: "100%",
                          background: "#1c2333",
            ...
            >
              ✅ Giảng viên và cán bộ (STAFF/OPERATOR/ADMIN) được miễn phí gửi
              xe
            </div>
          </div>
        </div>
          ...
          <div style={{ maxHeight: 500, overflowY: "auto" }}>
            {log.map((l) => (
              <div
                ...
                style={{
                  padding: "10px 16px",
                  borderBottom: "1px solid rgba(42,54,80,.4)",
                  display: "flex",
                  gap: 10,
                }}
              >
                <div
                  style={{
                    ...
                    height: 7,
                    borderRadius: "50%",
                    background: TYPE_COLOR[l.eventType] || "#3b82f6",
                    marginTop: 5,
                    flexShrink: 0,
                  }}
                />
                  ...
                  style={{
                    ...
                    background:
                      l.status === "OK"
                        ? "rgba(34,197,94,.15)"
                        : "rgba(245,158,11,.15)",
                    color: l.status === "OK" ? "#22c55e" : "#f59e0b",
                  }}
                >
                  {l.status}
                </span>
          ...
          >
            ...
            {modal === "create" && (
              <>
                ...
                <MField label="Mã HCMUT *">
                  <input
                    style={inputStyle}
                    value={form.hcmutId || ""}
                    onChange={(e) =>
                      setForm({ ...form, hcmutId: e.target.value })
                    }
              ...
              <>
                ...
                <div style={{ textAlign: "center", padding: "16px 0" }}>
                  ...
                  <div style={{ fontSize: 12, color: "#64748b" }}>
                    ({targetUser?.hcmutId}) — Tài khoản sẽ bị khóa, không thể
                    đăng nhập.
                  </div>
                </div>
                <ModalActions
            ...
            {modal === "reset" && (
              <>
                <ModalHeader
                  title={`Reset mật khẩu: ${targetUser?.hcmutId}`}
                  onClose={closeModal}
                />
    ...
    <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
      ...
      <button
        ...
        style={{
          ...
          padding: "10px",
          borderRadius: 8,
          background: saving ? "#374151" : confirmColor,
          border: "none",
          color: "#fff",
          fontSize: 13,
          fontWeight: 600,
